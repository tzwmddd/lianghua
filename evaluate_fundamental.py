"""估值因子 A/B 评估:对比 加/不加 pe_ttm/pb/pcf 的横截面选股信号质量。

口径与 recommend_q5.py 对齐:相对标签(未来30日是否跑赢沪深300)+ 相对 Rank IC。
A 组 = 22技术 + 3超额 + size(现状,无估值因子)
B 组 = A 组 + log_pe_ttm + log_pb + log_pcf(价值因子)
两者共用同一套中性化(逐列独立回归,故 A 的因子中性化结果两组一致)。
"""
import glob
import os
import sys
...
def main():
    ...
    all_style = BASE_STYLE + VAL_LOG_COLS
    pool = neutralize(pool, all_style)
    print(f"中性化完成(风格因子 {len(all_style)} 个 + size)", flush=True)

    feats_a = BASE_STYLE + ["size"]
    feats_b = BASE_STYLE + VAL_LOG_COLS + ["size"]

    print(f"\n>>>>>> A 组: 基线 {len(feats_a)} 特征(22技术+3超额+size)", flush=True)
    ic_a, ge_a = run_cross(pool, feats_a)
    ra = summarize("A 组(无估值因子)", ic_a, ge_a)

    print(f"\n>>>>>> B 组: +估值因子 {len(feats_b)} 特征", flush=True)
    ic_b, ge_b = run_cross(pool, feats_b)
    rb = summarize("B 组(+log_pe_ttm/log_pb/log_pcf)", ic_b, ge_b)
    ...
    path = os.path.join(RESULTS_DIR, "evaluate_fundamental_report.txt")
    with open(path, "w", encoding="utf-8") as f:
        f.write("==============================================================\n")
        f.write("估值因子 A/B 评估 (相对标签 + 相对 Rank IC)\n")
        f.write("==============================================================\n\n")
        ...
        f.write(f"B 组(+估值 {len(feats_b)} 特征): "
