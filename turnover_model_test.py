"""换手率因子接入模型验证(embargo 口径)。

换手率单因子 IC -0.077(低换手->高收益),是项目最强单因子。本脚本对比:
  tech_only         : 技术22+超额3
  tech_turnover     : + log_turnover + log_turnover_ma20
换手率特征纳入 neutralize(行业+市值中性化),embargo=45,class 标签,horizon=30。
"""
import glob
import os
import sys
...
TURNOVER_COLS = ["log_turnover", "log_turnover_ma20"]
...
CONFIGS = {
    ...
    "tech_turnover": {
        "style": BASE_STYLE + TURNOVER_COLS,
        "features": BASE_STYLE + TURNOVER_COLS,
        "desc": "技术+超额+换手率",
    },
}
...
def main():
    ...
    path = os.path.join(RESULTS_DIR, "turnover_model_test_report.txt")
    with open(path, "w", encoding="utf-8") as f:
        f.write("换手率因子接入模型验证 (embargo 口径)\n")
        f.write("====================================\n\n")
        f.write(f"horizon {FORECAST_HORIZON}, embargo {EMBARGO_DAYS}, 点-in-time 成分股\n")
        f.write("模型 num_leaves=31 / min_child=20 / lr=0.10\n\n")
        f.write("\n".join(lines))
        f.write("\n")
