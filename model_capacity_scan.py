"""模型容量/正则化扫描(embargo 口径)。

真实信号弱(IC~0.03),之前泄露口径下的结论是"欠拟合,加大容量(num_leaves=63)"。
正确口径下应验证相反假设:大模型是否在过拟合噪声,收缩模型(num_leaves 小 +
min_child_samples 大)是否更好。

固定 tech_only(技术22+超额3) + class 标签 + horizon=30 + embargo=45。
"""
import glob
import os
import sys
...
STYLE_COLS = FEATURE_COLS + EXCESS_COLS
FEATURES = STYLE_COLS

NUMS_LEAVES = [7, 15, 31, 63]
MIN_CHILD = [20, 500]
...
def run_cross(pool, num_leaves, min_child_samples):
    ...
    for t in rebal_dates:
        ...
        model = lgb.LGBMClassifier(
            n_estimators=150, learning_rate=0.10, num_leaves=num_leaves,
...
def main():
    ...
    lines = []
    header = (f"{'num_leaves':<12}{'min_child':<12}{'Rank IC':>10}{'ICIR':>8}"
              f"{'IC>0':>8}{'SE':>8}{'期数':>6}")
    print(header, flush=True)
    lines.append(header)
    for nl in NUMS_LEAVES:
        for mc in MIN_CHILD:
            ic = run_cross(pool, nl, mc)
            se = ic.std(ddof=1) / np.sqrt(len(ic)) if len(ic) > 1 else float("nan")
            row = (f"{nl:<12}{mc:<12}{ic.mean():>+10.4f}{icir(ic):>+8.3f}"
                   f"{(ic > 0).mean():>8.1%}{se:>8.4f}{len(ic):>6}")
            print(row, flush=True)
            lines.append(row)

    path = os.path.join(RESULTS_DIR, "model_capacity_scan_report.txt")
    with open(path, "w", encoding="utf-8") as f:
        f.write("模型容量/正则化扫描 (embargo 口径)\n")
        f.write("==================================\n\n")
        f.write(f"tech_only + class + horizon {FORECAST_HORIZON} + embargo {EMBARGO_DAYS}\n")
        f.write(f"n_estimators=150, lr=0.10, subsample/colsample=0.8\n\n")
        f.write("\n".join(lines))
        f.write("\n")
