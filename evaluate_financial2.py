"""现金流质量+增长因子增量评估:对比 5财务因子 vs 9财务因子。

验证新加的现金流质量(ocf_to_np/ocf_roa)与增长(net_asset_growth/total_asset_growth)
是否在原有财务因子(roe/np_growth/rev_growth/debt_ratio/gross_margin)基础上仍有增量。

口径与 evaluate_financial.py 对齐:相对标签 + 相对 Rank IC + 扩张窗口。
A 组 = 技术+超额+估值+size + 5个原财务因子
B 组 = 技术+超额+估值+size + 9个财务因子(原5 + 现金流质量2 + 增长2)
"""
import glob
import os
import sys
...
FIN_COLS_OLD = ["roe", "np_growth", "rev_growth", "debt_ratio", "gross_margin"]
...
def main():
    ...
    feats_a = BASE_STYLE + VAL_LOG_COLS + FIN_COLS_OLD + ["size"]
    feats_b = BASE_STYLE + VAL_LOG_COLS + FINANCIAL_COLS + ["size"]

    print(f"\n>>>>>> A 组: {len(feats_a)} 特征(5财务因子)", flush=True)
    ic_a, ge_a = run_cross(pool, feats_a)
    ra = summarize("A 组(roe/np_growth/rev_growth/debt_ratio/gross_margin)", ic_a, ge_a)

    print(f"\n>>>>>> B 组: {len(feats_b)} 特征(+现金流质量2+增长2)", flush=True)
    ic_b, ge_b = run_cross(pool, feats_b)
    rb = summarize("B 组(+ocf_to_np/ocf_roa/net_asset_growth/total_asset_growth)", ic_b, ge_b)
    ...
    path = os.path.join(RESULTS_DIR, "evaluate_financial2_report.txt")
    with open(path, "w", encoding="utf-8") as f:
        f.write("==============================================================\n")
        f.write("现金流质量+增长因子 增量评估 (相对标签 + 相对 Rank IC)\n")
        f.write("==============================================================\n\n")
